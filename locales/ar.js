export default {
    nav_home: "الرئيسية", nav_about: "من أنا", nav_projects: "مشاريعي", nav_skills: "مهاراتي",
    nav_gallery: "رحلتي بالصور", nav_hobbies: "الجانب الشخصي",
    nav_quran: "رحلتي مع القرآن",
    nav_media: "قرآن وأناشيد", nav_contact: "تواصل",

    hero_eyebrow: "بسم الله الرحمن الرحيم",
    hero_title: 'أنا <span class="text-gradient">خالد هشام</span>',
    hero_role: 'طالب بكلية الآداب <span class="sep">|</span> مهتم بالبرمجة والتقنية',
    hero_tagline: "طالب في كلية الآداب - جامعة بنها، بحب أطور نفسي في البرمجة، وبيني وبين الرياضة والفنون البصرية شغف مستمر.",
    meta_college_short: "طالب بكلية الآداب", meta_hafez: "حافظ للقرآن الكريم", meta_tech: "مهتم بالبرمجة والتقنية",
    hero_btn_projects: "مشاريعي", hero_btn_contact: "تواصل معي",

    about_subtitle: "تعرف عليا", about_title: "من أنا؟", about_kicker: "نبذة عني",
    about_p1: 'أنا <span class="highlight">خالد هشام</span>، طالب بكلية الآداب بجامعة بنها، مهتم بالبرمجة والتقنية، وبحب أطور نفسي وأجرب حاجات جديدة.',
    about_card_title: "بياناتي",
    info_name: "الاسم", info_name_val: "خالد هشام",
    info_study: "الدراسة", info_study_val: "كلية الآداب - جامعة بنها",
    info_major: "التخصص", info_major_val: "مساحة وخرائط رقمية",
    info_interests: "الاهتمامات", info_interests_val: "البرمجة، الرياضة، المونتاج",
    info_skills: "المهارات", info_skills_val: "البرمجة، تحرير الفيديو",
    info_city: "الدولة", info_city_val: "مصر",

    gallery_subtitle: "رحلتي", gallery_title: "من الطفولة للنهاردة",
    gallery_young: "وأنا صغير", gallery_university: "في الجامعة", gallery_old: "أنا دلوقتي",
    gallery_no_photo: "الصورة قيد الإضافة",

    hobbies_subtitle: "وقت الفراغ", hobbies_title: "هواياتي واهتماماتي",
    hobby_swim: "السباحة", hobby_code: "البرمجة", hobby_games: "الألعاب",
    hobby_football: "كرة القدم", hobby_edit: "المونتاج والإيديت", hobby_gym: "الجيم",

    projects_subtitle: "أعمالي", projects_title: "مشاريعي",
    projects_empty_title: "المشاريع قيد الإضافة",
    projects_empty_text: "بجهّز مشاريعي الحقيقية دلوقتي، وهتلاقيها هنا أول ما تكون جاهزة.",
    skills_subtitle: "قدراتي", skills_title: "مهاراتي",

    qj_subtitle: "علاقتي بالقرآن", qj_title: "رحلتي مع القرآن الكريم",
    qj_verse: "﴿ وَلَقَدْ يَسَّرْنَا الْقُرْآنَ لِلذِّكْرِ فَهَلْ مِنْ مُدَّكِرٍ ﴾",
    qj_verse_source: "سورة القمر - آية 17",
    qj_headline: "لحظة ختم القرآن الكريم",
    qj_text: 'بفضل الله، ختمت حفظ القرآن الكريم كاملاً في <span class="highlight">أول يوم من رمضان 1443هـ</span>، الموافق <span class="highlight">2 أبريل 2022م</span>، وكان عمري وقتها <span class="highlight">14 سنة</span> وأنا في <span class="highlight">الصف الثالث الإعدادي</span>. وحالياً بدرس أحكام التجويد استعداداً للحصول على الإجازة.',
    qj_hijri: "1 رمضان 1443هـ", qj_gregorian: "2 أبريل 2022م",
    qj_stage: "الصف الثالث الإعدادي", qj_tajweed: "بدرس التجويد حالياً",

    faith_subtitle: "وقفة إيمانية", faith_title: "آية - حديث - ذكر",
    tab_ayah: "آيات قرآنية", tab_hadith: "أحاديث نبوية", tab_zekr: "أذكار",

    media_subtitle: "استمع معايا", media_title: "قرآن - أناشيد",
    media_note: "لا تتجاهل استمع لعل هذا يدخلنا الجنة",
    media_quran_group: "تلاوات قرآنية", media_nasheed_group: "أناشيد",
    media_placeholder1: "سورة الفرقان / عمر عكاشه", media_placeholder2: "سورة نوح / عمر مطر", media_placeholder3: "سورة فاطر / أحمد الششتاوي",
    media_placeholder4: "غريب لست اعرفني", media_placeholder5: "قل للعيون اذا تساقط دمعها", media_placeholder6: "هل نلتقي ؟",
    media_tag_quran: "قرآن كريم", media_tag_nasheed: "نشيد",

    contact_title: "يسعدني تواصلك معايا",
    contact_p: "لو حابب تتكلم معايا أو تعرف أكتر عني، تقدر تتواصل معايا من هنا.",
    contact_wa: "واتساب", contact_fb: "فيسبوك", contact_ig: "إنستجرام",

    footer_tagline: "الموقع الشخصي",
    footer_copy: "© 2026 خالد. جميع الحقوق محفوظة.",

    aria_lang_toggle: "تغيير اللغة", aria_theme_toggle: "تبديل الوضع الليلي/النهاري",
    aria_mobile_menu: "فتح القائمة", back_to_top: "العودة للأعلى",
    aria_faith_prev: "السابق", aria_faith_next: "التالي", aria_faith_copy: "نسخ النص",
    aria_copy_phone: "نسخ رقم الهاتف",
    aria_lightbox_prev: "السابق", aria_lightbox_next: "التالي", aria_lightbox_close: "إغلاق",

    notfound_message: "الصفحة اللي بتدور عليها مش موجودة.",
    notfound_back: "العودة للرئيسية",

    toast_copied: "تم النسخ", toast_phone_copied: "تم نسخ الرقم",
    audio_error: "تعذر تحميل الملف الصوتي"
};
